...
## 17. ローカル UI で Cloud/GCS 結果を確認

Cloud Scheduler が GCS に保存した候補を、ローカル Growly Sales UI の**候補収集**タブで確認できます。

### ローカル .env 例

```env
GROWLY_CLOUD_RUN_SERVICE_URL=https://growly-sales-daily30-xxxxx.asia-northeast1.run.app
...
### Google 認証（ローカルから GCS 読み取り）

```bash
gcloud auth application-default login
```

認証がない場合、`GET /api/daily30-dashboard` は 503 と分かりやすいメッセージを返します（Secret 値は含めません）。

### UI 表示

- **Cloud Daily 30 実行結果** — collected / emailFound / status / mode
- **email_found 候補一覧** — URL クリック可、Lead化承認ボタン（GCS に書き戻し）
- 画面上部バナー — `保存先：Cloud Storage` に切り替わり

### Cloud Run ルート URL

Cloud Run では `GROWLY_CLOUD_RUN_API_ONLY=true` のため `/` は API 説明テキストのみ（営業OS UI は出しません）。ローカル UI は `npm run growly-sales:ui` で従来どおり利用します。
