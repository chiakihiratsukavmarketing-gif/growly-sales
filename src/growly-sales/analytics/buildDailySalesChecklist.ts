...
export function buildDailySalesChecklist(leads: Lead[], offer?: OfferProfile): DailyChecklistItem[] {
  ...
  const items: DailyChecklistItem[] = [
    ...
    },
    {
      ...
      order: 2,
      label: '診断希望Leadを確認する',
      description: '診断希望の返信を最優先で対応（レポートは自動作成しません）。',
    ...
    },
    {
      ...
      order: 3,
      label: '新規Lead候補を追加する',
      description: '候補収集タブで新しい会社を追加（下書き候補が尽きたら実施）。',
    ...
    },
    {
      ...
      description: '候補収集タブでメールアドレス付き候補を確認。',
      status: noDraftCandidates ? 'attention' : 'optional',
      targetTab: 'candidate-collection',
    ...
    },
    {
      ...
      order: 6,
      label: 'Gmail下書きを作成する',
      description: '下書き候補タブで1社ずつ Gmail 下書きを作成（自動送信なし）。',
    ...
    },
    {
      ...
      description: '返信状態・要約・フォロー予定日を記録。返信本文全文は保存しません。',
